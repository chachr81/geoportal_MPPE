...
class CircuitosController extends Controller
{
    ...
    public function circuitos_educativos($estado)
    {
                    ...
                    ( SELECT 
                        ...
                        ,array_to_json(array_agg(f)) As features
                            FROM 
                                (SELECT 
                                    \'Feature\' As type
                                    , row_to_json((SELECT l FROM (SELECT id, cod_cir) As l )) As properties
                                    , ST_AsGeoJSON(geom)::json As geometry
                                FROM circuitos_educativos_nacional where cod_cir like \''.$estado.'%\'
                                ) As f
                    )As fc
            ');
